...
from data.data_loader import ISRUC, DOD
from torch.utils.tensorboard import SummaryWriter
import seaborn as sn
import pandas as pd
import torcheeg
#from torcheeg.model_selection import KFoldCrossSubject, train_test_split_cross_subject
...
class Trainer(object):
    ...
    def setup_dataloaders(self):
        ...
        train_dir = os.path.join(self.cfg.base_path, self.ft_split)
        ...
        if 'isruc' in self.cfg.base_path:
            ...
            train_dataset = DOD(train_index, train_dir, n_channels=12, multilabel=False, scorer=self.cfg.dod_scorer, softlabels=True)
            test_dataset = DOD(test_index, test_dir, n_channels=12, multilabel=False, scorer=self.cfg.dod_scorer, softlabels=True, consensus=False)
            print("Scorer = ", self.cfg.dod_scorer)

        #val_dataset, test_dataset = train_test_split_cross_subject(dataset=dataset)

        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=self.cfg.batch_size, shuffle=True)
