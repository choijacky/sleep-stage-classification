...
def pretext_train_test(root_folder, k, N, epoch_sec, dest_folder, reduced, standard):
    ...
    #     sets.append(index_list[test_index].tolist())
    #     print(index_list[test_index])

    # for i in range(len(sets)):
    #     sample_process(root_folder, k, N, epoch_sec, str(i), sets[i], dest_folder, reduced, standard)
    ...
    for i, j in enumerate(index):
        if i % N == k:
            ...
            for idx in range(num_events):
                ...
                path = dest_folder + '/{}/'.format(train_test_val) + 'dodh-' + j + '-' + str(idx) + '--' + str(labels[0][idx]) + '--' + str(labels[1][idx]) + '--' + str(labels[2][idx]) + '--' + str(labels[3][idx]) + '--' + str(labels[4][idx]) + '--' + str(majority) + '.npy'

                wavelet = cwt(1 / 100, 3000, device='cpu')
                waveshape = (30, 60)
                s = wavelet(X[idx, 0, :], waveshape[1]).numpy()
