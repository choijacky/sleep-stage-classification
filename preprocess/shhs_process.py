...
def pretext_train_test(root_folder, k, N, epoch_sec):
    ...
    all_index = sorted([int(path[6:12]) - 200000 for path in os.listdir(root_folder + 'edfs/shhs1')])
    
    train_index = np.random.choice(all_index, int(len(all_index) * 0.98), replace=False)
    val_index = np.random.choice(list(set(all_index) - set(train_index)), int(len(all_index) * 0.01), replace=False)
    test_index = list(set(all_index) - set(train_index) - set(val_index))


    # print(len(all_index))
    # print ('start counting process!')
    # counting_process(root_folder, k, N, epoch_sec, 'pretext', all_index)
    ...
    sample_process(root_folder, k, N, epoch_sec, 'val', val_index)
    print ()
...
def sample_process(root_folder, k, N, epoch_sec, train_test_val, index):
    ...
    for i, j in enumerate(index):
        if i % N == k:
            ...
            data = mne.io.read_raw_edf(root_folder + 'edfs/shhs1/' + 'shhs1-' + str(200000 + j) + '.edf')
            X = data.get_data()
            ...
            with open(root_folder + 'annotations-events-profusion/shhs1/' + 'shhs1-' + str(200000 + j) + '-profusion.xml', 'r') as infile:
                text = infile.read()
            ...
            for slice_index in range(X.shape[1] // (125 * epoch_sec)):
                path = '/cluster/project/jbuhmann/choij/datasets/' + 'shhs_proc/{}/'.format(train_test_val) + 'shhs1-' + str(200000 + j) + '-' + str(slice_index) + '.pkl'
...
if __name__ == '__main__':
    ...
    if not os.path.exists('/cluster/project/jbuhmann/choij/datasets/shhs_proc/'):
        os.makedirs('/cluster/project/jbuhmann/choij/datasets/shhs_proc/train')
        os.makedirs('/cluster/project/jbuhmann/choij/datasets/shhs_proc/val')
        os.makedirs('/cluster/project/jbuhmann/choij/datasets/shhs_proc/test')

    root_folder = '/cluster/project/jbuhmann/choij/shhs/polysomnography/'
