...
class FrameBackBone(nn.Module):
    def __init__(self, fs: int, window, recon_mode, num_channels=1):
        super().__init__()
        self.recon_mode = recon_mode
        self.num_channels = num_channels
        if self.recon_mode == "masked_tokens":
            self.model = BackBone(input_size=int(fs * window), input_channel=num_channels, layers=[1, 1, 1, 1])
        else:
            self.model = WhackBone(input_size=int(fs * window), input_channel=num_channels, layers=[1, 1, 1, 1])
        self.feature_num = int(fs * window) #self.model.get_final_length() // 2
...
class WhackBone(nn.Module):
    def __init__(self, input_size, input_channel, layers):
        ...
        self.conv1 = nn.Conv1d(input_channel, 32, kernel_size=5, stride=1, padding=2, bias=False)
        self.bn1 = nn.BatchNorm1d(32)
        self.relu = nn.ELU(inplace=True)
        self.maxpool = nn.MaxPool1d(kernel_size=3, stride=1, padding=1)  # Adjusted stride
        ...
        self.maxpool3 = nn.AvgPool1d(kernel_size=10, stride=1, padding=0)  # To get [b, 64, 4]
        ...
        self.maxpool5 = nn.AvgPool1d(kernel_size=5, stride=1, padding=0)  # To get [b, 64, 2]
        ...
        self.layer7x7_3 = self._make_layer7(BasicBlock7x7, 48, layers[2], stride=1)
        self.layer7x7_4 = self._make_layer7(BasicBlock7x7, 64, layers[3], stride=2)
        self.maxpool7 = nn.AvgPool1d(kernel_size=7, stride=1, padding=0)  # To get [b, 64, 1]
